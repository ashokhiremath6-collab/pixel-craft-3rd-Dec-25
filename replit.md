...
- **Convert to Photorealistic (Renders page)**: A dedicated card at the bottom of the Renders page lets users upload a SketchUp screenshot or any 3D render image, optionally describe design alterations (material/colour/finish changes only), and convert it to a photorealistic interior photograph using Gemini. Result can be previewed and saved directly to any project in the renders library. Backend: `POST /api/ai-renders/photoreal` now accepts an optional `alterationNotes` field.

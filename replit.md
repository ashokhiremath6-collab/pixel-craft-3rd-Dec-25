...
- **Design Intelligence Chat**: AI assistant powered by Claude Sonnet (claude-sonnet-4-6) via Replit Anthropic integration for full-spectrum interior design queries. Supports multimodal input (images, PDFs). Accessible by Admin and Designer roles. Features: Generate Render Brief (transfers conversation to AI Renders page), Floor Plan generator (SVG output, downloadable, 1:50 scale).
